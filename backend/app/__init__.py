...
def generate_icon(config: IconConfig, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Генерируем SVG
    svg_code = SVGEngine.generate_icon(config.style, config.shape, config.color)
